...
- sort=recommended には閲覧者自身の投稿も含める。本人の投稿は他の利用者の候補とは別に、先頭ページの取得時に新しい順で最大3件だけ候補へ加える（本人の投稿だけで候補が埋まらないようにする）。本人の投稿は未読として優先せず、他の候補と同じ分散・新しさの基準で並べ、reasonCode=own_post を返す
