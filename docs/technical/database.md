...
| concern_clusters | id, legacy_label, legacy_summary, label, summary, status, model_version, created_at, updated_at | AI が作った分類。`legacy_*` は既存外部キーを保ったまま移行するための互換用必須列。statusはpending/generating/ready。生成中はupdated_atをclaim lease時刻として使う。画面表示前に長さ・禁止表現・個人情報パターンを検査 |
