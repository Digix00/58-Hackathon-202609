...
| concern_clusters | id, legacy_label, legacy_summary, label, summary, status, model_version, created_at, updated_at | AI が作った分類。`legacy_*` は既存外部キーを保ったまま移行するための互換用必須列。新規クラスタのlabel/summaryは生成前にNULL。画面表示前に長さ・禁止語・個人情報パターンを検査 |
...
`concern_clusters` は既存の `concerns.cluster_id` 外部キーを保つため、既存の必須カラムをテーブル再作成で変更しない。既存カラムを `legacy_label` / `legacy_summary` として残し、アプリケーションが使う nullable な `label` / `summary` を追加する。新規クラスタでは互換用カラムへ `__pending__` を入れ、表示用label・summaryを検証して保存するまでstatusをpendingに保つ。`model_version` は従来どおりEmbedding model versionを保持する。
