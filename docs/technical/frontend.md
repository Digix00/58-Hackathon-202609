...
- `en` 選択時はナビ、設定、投稿、フィード、詳細、クイズ、履歴、初期登録、ログイン案内、管理画面の固定文言を英語にする。`original` と `jaHira` のUI文言は日本語のままとし、`jaHira` の投稿本文・地域表示は既存のひらがな対応を使う。
- UI文言は `frontend/src/i18n/messages.ts` に日英の組として定義する。コンポーネントは `useTranslation().t()`、ViewModelなどの純粋関数は `translate(language, key)` を使う。数値入りの文章も一文単位で定義し、`aria-label`、状態通知、入力例、バリデーションも対象とする。
...
- クイズ解説の既存の固定文はUI辞書で英語化する。任意の解説・クラスタ名など英語版を取得できないサーバー生成文は原文を保持する。フロントエンドから翻訳サービスを呼ばない。
