...
投稿EmbeddingはD1へ保存せず、Cloudflare Vectorizeのconcern indexへ保存する。VectorizeのIDはconcern ID、metadataはcluster IDのみとする。D1のconcerns.cluster_idを正とし、投稿の処理状態がreadyになるまでフィード上のcluster割当を公開しない。Vectorizeは非同期クラスタリングの内部検索専用であり、利用者向けの自由入力検索やRAGには使わない。
