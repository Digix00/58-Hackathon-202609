...
| `concern_views` | concern_id、actor_key、viewed_at | 既読と推薦に利用。actor_key は認証済みユーザーに対応する内部 users.id |
...
`user_id` はサーバーがLINEログイン済みセッションから解決する内部の `users.id` であり、リクエストから受け取らない。`concern_views.actor_key` も同じ内部 `users.id` を参照し、LINE user ID を直接保存しない。通常ブラウザおよび未ログインのLINEミニアプリによる公開投稿の閲覧では、`user_id`、既読、リアクション、クイズ回答、学習イベントを記録しない。
