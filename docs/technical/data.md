...
推薦v3は `concern_views` を投稿ごとに集計し、投稿者自身を除くユニークなログイン済み閲覧者数を利用する。既存の `(concern_id, actor_key)` 一意制約で再表示・再送の二重加算を防ぐ。年代・性別・都道府県の入力有無によって数え方を変えない。新しい追跡用テーブルや匿名ユーザー識別子は追加しない。

`feed_impressions` はAPIが返した候補の取得履歴であり、実表示の保証はないため公平な表示機会の集計には使わない。新しい `algorithm_version` は `v3`、推薦理由は `familiar_theme`、`discovery`、`less_heard` を追加する。閲覧者数は内部候補の `readerCount` にだけ渡し、公開APIには返さない。詳しくは [推薦設計](./recommendation.md) を参照する。
