...
- D1の条件付き更新でclusterをpendingからgeneratingへ原子的にclaimする。同じclusterを別workerが生成中ならQueueを再試行する。競合した新規投稿は翻訳・Embedding・割当結果をreadyで保存し、clusterの要約がreadyになるまではclusterをフィードに返さない。claimは5分で失効し、生成失敗時はpendingへ戻してQueueを再試行する。
