...
投稿カードの本文とリアクションの間に、推薦理由をテキストで表示する。色や装飾だけで意味を伝えず、文字拡大時は折り返す。通常Web・未ログインは「新着」、推薦処理のフォールバックは「新着順」とする。

- 身近なテーマの枠: 「最近読んだテーマの、別の声」
- 未知のテーマの枠: 「まだ出会っていない声」
- 閲覧機会を補う枠: 「まだ届いていない声」

「未読のテーマ」は未読投稿にクラスタがある場合の補完理由であり、テーマ全体が未読であることを保証しない。テーマ・地域の一時休止や読み方の選択UIは今回の実装に含めない。
