...
| 画面 | ルート | 利用条件 | 優先度 | 目的 |
| --- | --- | --- | --- |
| 起動 | 起動時 | 通常ブラウザ、LINEミニアプリ | MVP | 閲覧を開始する |
...
設定で英語を選ぶと、画面の固定文言、入力例、エラー、読み上げ用ラベルを英語に切り替える。投稿本文は既存APIの英語表現を表示し、翻訳待ち・失敗の場合は案内を添えて原文を表示する。日本語・ひらがな選択時のUIは日本語を維持する。

表示言語はログインユーザーの既存設定へ保存し、再読み込み後にも復元する。保存できなかった場合は元の言語を維持する。地域などの属性キーは変更せず、既存の表示名を利用する。任意に生成されたテーマ名・解説は、APIに英語版がない場合は原文で表示する。
