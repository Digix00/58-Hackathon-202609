...
LIFFの初期化に失敗した場合は、通常Webと同じ読み取り専用モードへ切り替える。公開フィードと投稿詳細は新着順で閲覧でき、投稿などの操作を選んだ場合はLINEミニアプリで開き直す案内を表示する。
...
通常WebとLINEログイン前のLIFFでは、新着順のフィードを表示する。LINEログイン済みのLIFFでは、未読を優先しながら「最近読んだテーマの別の投稿」「最近読んでいないテーマ（未分類を含む）」「実際の閲覧者数が少ない投稿」の3枠を繰り返す。身近な悩みから異なる声へ関心を広げ、届く機会の少ない声にも出会えることを目的とする。

推薦は新着順で取得した候補群の中で行い、異なるクラスタが残っていれば同じクラスタの連続を避ける。該当するテーマがない場合は通常の候補で補い、履歴なし・全件既読・候補不足でも閲覧を継続する。ページをまたいでも枠の順序を維持する。

属性未回答やAI処理前・失敗による未分類を理由に未読の重みを下げない。本人の属性との一致やリアクション数は順位に使わず、本文からセンシティブな属性を推測しない。都道府県・性別は利用者が指定した絞り込みにのみ使う。推薦理由を投稿カードに表示する。

閲覧者数はログイン済み利用者の実際の投稿表示を一人につき一度数え、投稿者自身とAPIの先読みを除く。匿名Webの閲覧数や滞在時間は収集しない。AI処理が利用できなくても未分類の原文で推薦を続け、推薦に必要な取得処理が失敗した場合は新着順相当へ切り替える。選定の詳細・制約は [推薦設計](../technical/recommendation.md) を参照する。
