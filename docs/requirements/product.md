...
- AIが生成した表示用ラベルは形式と長さを検証し、投稿本文の自動判定には利用しない
...
管理画面は作らない。投稿内容に対する自動判定、人手確認、公開保留は今回実装しない。入力形式と文字数の検証を通った投稿は、保存直後から `published` として扱う。
...
このサービスは医療、法律、緊急通報などの専門窓口の代替ではない。投稿の緊急性を判定したり、投稿者へ個別に対応したりする機能は提供しない。ハッカソンのデモでは実在の個人が特定できる内容を使わない。
