...
通常WebとLINEログイン前のLIFFでは、新着順のフィードを表示する。LINEログイン済みのLIFFでは、利用者に紐づく履歴を使った推薦順のフィードを表示する。推薦では直近の投稿、未読投稿、テーマの分散、都道府県・年代の分散を組み合わせ、既読クラスタの偏りを避ける。利用者が登録した都道府県と同じ県・同じ地方の悩みも、1ページの一部に限って優先し、近くの悩みに出会えるようにする。開かれずに繰り返し表示された投稿は下げ、日によって並びを少し変えて、同じフィードに飽きないようにする。
