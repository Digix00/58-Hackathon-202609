...
| 区分 | 意味 |
| --- | --- |
| MVP | LINEミニアプリ（LIFF）で投稿・閲覧・リアクションを成立させるために必ず動作させる機能 |
...
LINEミニアプリから寄せられた悩みを、年齢や所属コミュニティを越えて知る機会を作る。LIFF内のWeb画面、LINE入力、音声入力という複数の入口と、クラスタリング、レコメンド、クイズという複数の知り方を用意し、発信が得意でない人も参加できる状態を作る。
...
MVPでは、LINEミニアプリ（LIFF）で認証済みユーザーの投稿と閲覧が完結する状態を必須とする。

1. LINEミニアプリのWeb画面を開く
2. LIFFで認証済みユーザーとして投稿する
3. D1に投稿を保存する
4. 投稿一覧から他人の悩みを読む
5. 読んだ悩みにリアクションする
...
MVPに加えて、次の機能を今回の完成条件に含める。デモでは、LIFFとLINEをつなぎ、他人の悩みを知る体験が最後まで成立することを示す。

1. 意味の近い悩みをクラスタリングし、フィード上に「同じことで困っている人がいる」ことを表示する
2. クラスタ、未読履歴、投稿の新しさを使い、次に読む悩みをレコメンドする
3. 3ユーザーの属性と3件の実投稿を使った対応付けクイズを、LIFFとLINEから体験できるようにする
4. LINE公式アカウントの友だち登録、投稿受付、デイリークイズの一斉配信を行う
5. 音声入力、読み上げ、英語翻訳、ひらがな表示を行う
6. LINEユーザー単位で、読んだ悩みとクイズ結果の学習履歴を表示する
...
1. ユーザーが投稿画面を開く
2. LIFF内のWeb入力、LINE入力、音声入力のいずれかを選ぶ
...
5. 未読状態、クラスタ、地域をもとにレコメンドされた次の悩みを表示する
6. 次の投稿へ移動する
...
クイズに使う3ユーザーは、公開済みの投稿を持つ異なるLINEユーザーから抽出する。表示する属性は年代、性別、地域の広い区分に限り、正確な年齢や個人を特定できる情報は表示しない。3件の悩みはすべて実際の投稿であり、ユーザー情報と悩みの表示順をそれぞれランダム化する。
...
学習履歴は LINE ユーザー単位で保持し、users.deleted_at が設定されたユーザーは本人の履歴を参照できないようにする。個人を特定する情報や投稿者の実名は履歴に表示しない。
...
- カーソルを使って次の投稿を取得できる
- 投稿を既読として記録できる
- 未読、テーマ分散、投稿の新しさを使ってLINEユーザー向けフィードを生成する
...
- 配信処理を再実行しても同じ日付の配信要求を重複実行しない
- LINEから受け取った投稿を公開上は匿名の投稿として保存できる
...
- users.deleted_at が設定されたユーザーの履歴は参照できなくする
- LINEユーザーの履歴を user_id に紐づけて参照できる
...
- クイズでは3ユーザーの属性だけを表示し、LINE user IDや内部 user_idを表示しない
...
- [ ] LINEミニアプリで認証済みのユーザーが投稿できる
- [ ] 投稿した内容が、別のLINEユーザーのフィードに表示される
- [ ] 空本文、長すぎる本文、不正な属性をサーバーが拒否する
- [ ] 投稿へリアクションでき、同じユーザーで二重加算されない
...
1. LINEミニアプリを開き、LIFF認証後に公開上は匿名で利用できることを示す
2. ユーザーAがLIFFのWebまたは音声入力で「食堂が混んでいて昼休みに休めない」という悩みを投稿する
3. ユーザーBが別のLINEユーザーとしてフィードを開き、投稿と年代・地域の表示を見る
