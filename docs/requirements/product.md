...
この文書は、58ハッカソンで完成させる「Qiite」の目的、利用体験、実装範囲、技術要件、受け入れ条件を定める。実装時はこの文書を最上位の基準とする。
...
3. リアクション済みの投稿で同じ操作を選ぶとリアクションを解除する
4. 投稿カードの集計数を更新する
5. 同じ操作を繰り返した場合は二重加算しない

リアクションは返信ではなく、「読んだ」「考えた」「共感した」ことを示す集計情報として扱う。MVPのAPI値は `empathy` の一種類とし、画面上の名称は「そっと寄りそう」に統一する。

### UC-04 次に読む悩みを推薦する

通常WebとLINEログイン前のLIFFでは、新着順のフィードを表示する。LINEログイン済みのLIFFでは、利用者に紐づく履歴を使った推薦順のフィードを表示する。推薦では直近の投稿、未読投稿、テーマの分散、都道府県の分散を組み合わせ、既読クラスタの偏りを避ける。
...
- 配信済み、未配信、配信失敗の表示
- LINEからWebクイズへ戻る導線
- 投稿者へ、前回の通知以降に届いた寄りそいの人数と地域を知らせる寄りそい通知（毎日 20:00 JST と手動実行）
