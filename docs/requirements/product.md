...
未ログイン時は新着順またはランダムなフィードを返す。利用者に紐づく履歴がある場合は、直近の投稿、未読投稿、テーマの分散、地域の分散を組み合わせ、既読クラスタの偏りを避ける。
...
クイズに使う3ユーザーは、公開済みの投稿を持つ異なるLINEログイン済み利用者から抽出する。表示する属性は年代、性別、地域の広い区分に限り、正確な年齢や個人を特定できる情報は表示しない。3件の悩みはすべて実際の投稿であり、ユーザー情報と悩みの表示順をそれぞれランダム化する。
...
3. ユーザーBが通常ブラウザからフィードを開き、匿名の投稿と年代・地域の表示を見る
