...
flowchart TB
  ...
  Slot["紙の上端の切り欠き<br/>手紙ごとに横の持ち場が違う"]
  Tray["カードの上にある手元のしおり<br/>残っているぶんだけ"]
  Tabs["出したままの付箋<br/>挟み終えたぶんだけ"]
  Closed["閉じたノート<br/>付箋はその場に残る"]
  Send["これで出す"]
  Result["1組ずつ結果・解説"]
...
| いま読む手紙 | 縦にゆとりを持たせた本文と、紙の上端から外へ出した切り欠き。横の位置は手紙ごとにずらす | 切り欠きにしおりを差し込む。差し込んだしおりを押すと手元へ戻る |
| 手元のしおり | しおりの外形は保ち、直下に年代、性別、都道府県を大きく表示する。LINEプロフィール情報や名前は出さない | つまんで切り欠きへ運ぶ。押すだけでも差し込める |
| 挟み終えた付箋 | めくって下に入った手紙の付箋も、紙の上端から出したままにする | 押すと、その手紙へ移る |
| 閉じたノート | 3通すべてに挟み終えると閉じる。付箋は動かず、表紙だけが降りてくる | 付箋を押すと、その手紙が開いて読み直し・挟み替えができる |
| 送信 | 3通すべて埋まったときだけ現れる「これで出す」 | 回答を送信する |
| 結果 | 書き手の条件が挟まった紙と、短い判定・解説 | めくって1組ずつ読む。最後に履歴への導線を置く |

属性に含まれる都道府県名は、設定で選んだ表記（原文・ひらがな・英語）に合わせる。「回答しない」の表示は変えない。
...
付箋は紙の上端から外へ出して置く。紙に隠れるのは差し込んだ下端だけにして、条件の字は紙の外で読めるようにする。隠れる下端には字を置かない。この置き方は、いま開いている紙に挟んだ1枚も、めくって下に入った紙の付箋も、閉じた本から出ている付箋も、すべて同じにする。横の持ち場も高さも変えないので、紙をめくっても本を閉じても付箋は動かない。位置が動くと、自分が挟んだ1枚ではなく、あとから付けられた見出しに見える。差し込む深さで位置を決めるため、条件が長く折り返して付箋が伸びても、紙の上端に揃ったまま外側へ伸びる。

挟み終えた付箋は、その紙がめくられて下に入っても出したままにする。3枚がいつも見えていれば、どの手紙にどの条件を挟んだのかを見比べられる。いま開いている紙の付箋はその紙が持ち、押すと手元へ戻る。下に入った紙の付箋は紙より奥に置き、押すとその手紙へ移る。同じ場所に二重には出さない。外すための1枚と、移るための1枚が重なってしまう。

3通すべてに挟み終えると、一拍おいてノートが閉じる。付箋はその場に残り、表紙だけが降りてくる。手元のしおりを使いきると置き場は空になり、そのとき「これで出す」が出るぶん紙束が上がって、付箋が置き場の高さに来る。棚のふちが付箋を横切るので、空になったあいだは、ふちだけを消して高さはそのまま残す。付箋を押すとその手紙が開き、読み直しも挟み替えもそこから続けられる。閉じたノートは縮めない。読みはじめる前の表紙だけを小さく置く。付箋の並ぶノートを縮めると、条件の字が読めなくなる。読み終えて閉じた表紙からは、届いたことを告げる紙飛行機を降ろし、一言を読み終えたあとのものに差し替える。まだ次が届くという誤った合図にしないためで、空いた上辺はそのまま付箋の場所になる。
...
- 出したままの付箋はボタンとして扱い、何通目の手紙のものかと、挟んである条件を読み上げに含める。
