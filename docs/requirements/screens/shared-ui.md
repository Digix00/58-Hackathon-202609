...
| 下部ナビ | 「読む」「クイズ」「投稿」「履歴」「設定」の5項目。投稿を中央に置き、紙を丸く切り抜いたボタンとして帯の上辺からせり上げる |
| 設定 | 下部ナビの右端に置く。画面移動ではなく設定シートを開く操作として扱う |
| コンテンツ幅 | 360px以上の1カラムを基準に、左右16〜20pxの余白を取る |
| 現在地 | 下部ナビで、色とラベル・選択状態を組み合わせて示す |

下部ナビは画面の下端に常時表示する。本文の上端は端末の安全領域から測り、中央の「投稿」がせり上がるぶんの余白を本文の下端に確保する。内容が画面高を超える場合は下部ナビより上の本文領域を縦スクロールできるようにし、文字サイズの拡大やソフトウェアキーボード表示でも操作を隠さない。

## 設定シート

表示設定とログイン済みユーザーの個人設定は独立ページではなく、下部ナビの「設定」から開くボトムシートにまとめる。
