...
| 下部ナビ | 「読む」「クイズ」「投稿」「履歴」「設定」の5項目。投稿を中央に置き、他の項目と高さを揃えて淡い紫の背景で示す。文字サイズを大きくしてもナビの縦幅は変えない |
| 設定 | 下部ナビの右端に置き、`/settings` へ移動する |
| コンテンツ幅 | 360px以上の1カラムを基準に、左右16〜20pxの余白を取る |
| 現在地 | 下部ナビで、色とラベル・選択状態を組み合わせて示す |

下部ナビは画面の下端に常時表示し、本文との境界を細い直線で区切る。文字サイズを大きくしても、ナビの操作領域と縦幅は標準表示から変えない。本文の上端は端末の安全領域から測る。内容が画面高を超える場合は下部ナビより上の本文領域を縦スクロールできるようにし、文字サイズの拡大やソフトウェアキーボード表示でも操作を隠さない。
...
| 都道府県名の表記 | 原文・ひらがな・英語 | LINEログイン後にアカウントへ保存し、投稿・クイズの都道府県名と、プロフィール・フィードの地域選択肢へ反映する |
