...
フィードAPIの `sort` は実行環境とLINEログイン状態から決める。

- LINEミニアプリ内でLINEログイン済みの場合は `sort=recommended` を指定する。
- 通常WebとLINEミニアプリ内の未ログイン状態では `sort=newest` を指定する。
- LIFFの認証状態を確認中は、状態が確定してから最初のフィード取得を行う。これにより、ログイン済み利用者へ一時的に新着順を取得してから推薦順へ切り替えることを避ける。
...
表紙のまわりには、クレヨンのシールを本の下に敷く。星・ハート・ロケット・みかづき・にじ・おはななど種類を変え、大きさ・色・傾きを1枚ずつ変えて本を囲むように散らす。1枚は紙のふちにかかるほど大きく取り、紙に潜った側は隠れたままにして、はみ出した側だけを見せる。全部が見える大きさに切りそろえると、本の下に貼った紙ではなく背景の模様に見える。左右のシールは画面の端で切れてよいが、本文の余白の途中では切らない。断ち落としに見える。ふもとの「めくってみる」の左右にもシールを回し、絵を本の外まで続ける。シールは紙の上には置かない。表紙の絵と重ねると、題字と一言より先に見てしまう。

シールは紙と一緒に置き、表紙が大きく広がるときはシールも一緒に大きくなる。紙だけが大きくなると、シールは貼った場所に取り残されて背景の模様に見える。「めくってみる」を選ぶと、シールは貼った場所から本の外へ散って消える。散る順は左右の1枚から始め、広がる紙に飲まれないようにする。散り終わりは表紙をめくり終える時間の中に収め、最初の悩みへは何も持ち越さない。

シールは貼ったまま、ずっと小さく揺れている。振れ幅・周期・位相は1枚ずつ変え、同じ間で動く列にしない。全部を同じに揺らすと、貼られたシールではなく、風で揺れる一枚の貼り紙に見える。きらり・ほしは揺れるかわりにときどき光り、絵の違いを動きにも出す。
...
- 本の下のシールは装飾として扱い、押せる要素にも読み上げの対象にもしない。動きを控える設定では、揺れも光も散りも出さず、表紙の切り替えと同時に消す。
