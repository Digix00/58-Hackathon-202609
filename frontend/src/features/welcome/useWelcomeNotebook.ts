...
/** 表紙を開くときに紙束が大きくなる時間。フィードと同じ手つきにそろえる。 */
...
type WelcomeStatus = 'idle' | 'saving' | 'failed'
...
type WelcomeAction =
  | { type: 'fieldChanged'; update: WelcomeFieldUpdate }
  | { type: 'lifting' }
  | { type: 'next'; turning: WelcomeTurn | null; update?: WelcomeFieldUpdate }
  | { type: 'back'; index: number; turning: WelcomeTurn | null }
  | { type: 'turningFinished' }
  | { type: 'saveStarted' }
  | { type: 'saveFailed'; message: string }

const initialWelcomeState: WelcomeState = {
  draft: emptyWelcomeDraft,
  index: 0,
  lifting: false,
  opened: false,
  turning: null,
 ...
 */
function settledIndex(state: WelcomeState): number {
  return state.turning?.settleTo ?? state.index
}

function applyUpdate(draft: WelcomeDraft, update: WelcomeFieldUpdate | undefined): WelcomeDraft {
 ...
 *   OS の動きを減らす設定でのめくり省略。
 */
export function useWelcomeNotebook() {
  ...
  const currentPage = WELCOME_PAGES[settled]
  /** 手前に見えている紙。戻りの紙が降りている間は、まだ前の紙が残っている。 */
  const facePage = WELCOME_PAGES[index]
  ...
  useEffect(() => {
    ...
    // 表紙が大きくなりきるのを待ってからめくる。押し上げとめくりを重ねない。
  ...
  const advance = useCallback(
    (startAngle: number, update?: WelcomeFieldUpdate) => {
  ...
  const updateField = useCallback((update: WelcomeFieldUpdate) => {
